...
- **(C)** Modules separated from core: dropped the redundant `Military ECM suite` MODULE (EW lives only in the core ECM slot; the L4862 ECM *part* stays); Modules panel reframed as special bolt-ons.
- **(D)** Stygium cloak reworked: threshold-gated by **`M.hullMax`** (="Medium" → freighter-scale-and-smaller, bars cruisers/capitals; loosened from "small" per James so the Medium-framed Ebon Hawk qualifies), size S / mass 1, **150 draw**, refuelable **stygian-crystal cloak-timer** (`adjModuleCharge`, `crystalMin` ~10 min/crystal), installs powered-down. + **Neuralyser array** module.
- **`hpLock`** flag added to migrate — pins authored hull HP (skips the class-conform). Fixes a **fleet-wide latent clobber**: the HP-conform loop was resetting every party ship on render (Ebon 1100→90, Phantom 1080→200, Mantis 720→90).
- **Live Firebase merges (tools/gm_push.py · project kotor-gurps / db kotor):** the live Ebon Hawk was far ahead of the repo — renamed "Ebon Hawk-class", **2400 HP via a Medium Freighter frame**, full 12-slot core, **quad lasers + concussion missiles**. Did a NON-destructive merge: added 2× chaff + Neuralyser + powered-down stygium cloak (40 crystal-min) to its empty module slot, preserved everything else (no hpLock needed — a core Hull frame self-stabilises HP). Restored + hpLock'd **chatni** (→1080) and **sekan** (→720), both of which had already reset to 90 live. Re-synced repo `truman.json` from live (was a stale Dynamic-class/1100 snapshot).

**KEY LEARNING — repo character JSONs are STALE backups; live Firebase is authoritative.** Never push a repo char JSON wholesale to Firebase without GET-diffing first (live is ahead). Use per-key field-merge (`gm_push.py patch <id> {"vehicles":…}`) and always read live before writing.

**FOLLOW-UP (flagged, not fixed):** the migrate **DR/shield-conform** loop (sheet.html ~L5018-5023) has the *same* latent-clobber pattern as HP, for any ship whose name matches a `SHIPS_FULL` entry. **chatni** ("X-70B Phantom-class prototype") matches → its authored DR 62 / shield 85 are conformed to **12 / 425** on every render (this session restored HP only, per the ask). If unwanted: add a DR/shield lock like `hpLock`, rename the ship out of `SHIPS_FULL`, or fix the catalogue entry.
